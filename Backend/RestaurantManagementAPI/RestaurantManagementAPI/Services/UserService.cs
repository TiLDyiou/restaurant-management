...
namespace RestaurantManagementAPI.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<ServiceResult<object>> GetUserProfileAsync(string username)
        {
            ...
            if (user == null) 
                return ServiceResult<object>.Fail("Không tìm thấy user");
        ...
        public async Task<ServiceResult<object>> UpdateUserAsync(string requesterUsername, bool isAdmin, string? targetMaNV, UpdateUserDto dto)
        {
            ...
            try
            {
                ...
                if (user == null) 
                    return ServiceResult<object>.Fail("User không tồn tại");
                ...
                return ServiceResult<object>.Ok(
                    new { user.TenDangNhap, user.Email }, 
                    emailChanged ? "Cập nhật thành công (Cần verify email)" : "Cập nhật thành công"
                );
            }
            catch (Exception ex)
            {
        ...
        public async Task<ServiceResult> VerifyEmailOtpAsync(string email, string otp)
        {
            ...
            if (user.OTP?.Trim() != otp.Trim() || user.OTPExpireTime < DateTime.UtcNow) 
                return ServiceResult.Fail("OTP lỗi hoặc hết hạn");
        ...
        public async Task<ServiceResult> ResendEmailOtpAsync(string email)
        {
            ...
            if (user == null) 
                return ServiceResult.Fail("Email không tồn tại");
            if (user.IsVerified) 
                return ServiceResult.Fail("Đã xác thực rồi");
            ...
            try 
            { 
                await _emailService.SendEmailAsync(user.Email!, "OTP Verify", $"OTP: {otp}"); 
            }
            catch (Exception ex) 
            { 
        ...
        public async Task<ServiceResult<object>> ToggleUserStatusAsync(string maNV)
        {
            ...
            if (nv == null) 
                return ServiceResult<object>.Fail("Không tìm thấy NV");

            nv.TrangThai = nv.TrangThai == "Đang làm" ? "Đã nghỉ" : "Đang làm";
            if (nv.TaiKhoan != null) 
                nv.TaiKhoan.IsActive = (nv.TrangThai == "Đang làm");

            await _context.SaveChangesAsync();
            return ServiceResult<object>.Ok(new { 
                nv.TrangThai, 
                Active = nv.TaiKhoan?.IsActive 
            });
        }
